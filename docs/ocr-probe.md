...
1. 按进程路径筛选目标进程实例。
2. 从 `MainWindowHandle`、同进程顶层窗口、主窗口后代窗口中收集候选窗口。
3. 过滤明显无关的辅助窗口，例如通知图标宿主、系统消息窗体、IME 窗体和 `settingWindow`。
4. 按可见性、最小化状态、默认区域是否可容纳、类名特征和窗口面积对候选窗口打分。
5. 在 `window` 模式下优先通过窗口句柄抓图；如果句柄抓图失败且窗口可见且未最小化，再退回 `screen` 抓图。
6. 对截图执行 OCR，输出原始文本、归一化文本、关键词结果、三态状态结果和候选窗口尝试明细。
