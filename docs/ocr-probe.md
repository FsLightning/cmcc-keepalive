...
- 默认抓图模式：`window`
- 默认 OCR 语言：`zh-CN`
- 默认关闭窗口预处理：不强制最大化或恢复普通窗体
- 默认普通窗体目标区域：`(120, 80) 1600x900`
- 默认状态检测区域：相对客户端区域 `(120, 220) 1200x700`
- 默认开启自适应区域：按当前 client 尺寸缩放检测区域
- 自适应基线 client：`2032x1074`
- 当首轮识别文本为空且未命中状态时，会自动触发一次“窗口预处理 + 重试”
...
5. 为每个候选窗口生成多套区域策略：`fixed`、`adaptive-scaled`、`fit-to-client`。
6. 在 `window` 模式下优先通过窗口句柄抓图；如果句柄抓图失败且窗口可见且未最小化，再退回 `screen` 抓图。
7. 对每个区域策略执行 OCR，按状态命中、关键词命中、文本长度和窗口评分综合选出最佳结果。
8. 如果首轮最佳结果仍为空文本且未命中状态，自动触发一次窗口预处理重试，并与首轮结果统一比较后选出最终结果。
...
- `capture`：抓图模式、命中的区域策略，以及最终使用的相对/绝对坐标。
...
- `attemptedWindows` 中的 `regionStrategy` 与 `result` 可用于区分不同区域策略下的抓图/OCR效果。
...
- 自适应区域能降低窗口尺寸变化带来的偏移，但当页面布局发生大幅改版时，仍需要重新采样并调整基线或固定区域。
...
```powershell
dotnet run --project .\src\OcrProbe\OcrProbe.csproj -- --adaptive-region true --baseline-client-width 2032 --baseline-client-height 1074
```
